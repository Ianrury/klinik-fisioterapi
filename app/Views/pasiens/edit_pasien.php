...
<div class="bg-white p-6 rounded-lg shadow-lg">
    <div class="bg-transparent border-b-4 border-primary w-1/4">
        <h2 class="text-primary font-bold text-xl">Edit Pasien</h2>
    </div>

    <!-- Form untuk menambah user baru -->
    <form action="<?= base_url('/pasiens/update/' . $pasien['id']); ?>" method="post">
        <?= csrf_field(); ?>
        <?php $validation = \Config\Services::validation(); ?>

        <div class="mt-5">
            <label for="nama" class="block text-lg font-semibold text-gray-900">Nama Pasien atau Atlet</label>
            ...
            <small class="text-red-500"><?= $validation->getError('nama'); ?></small>
        </div>
        ...
        <div class="mt-5">
            ...
            <div class="relative mt-1">
                <select name="j_kelamin" id="j_kelamin"
                    ...
                    <option value="" <?= old('j_kelamin', $pasien['j_kelamin']) == '' ? 'selected' : '' ?>>-- Pilih --
                    </option>
                    <option value="L" <?= old('j_kelamin', $pasien['j_kelamin']) == 'L' ? 'selected' : '' ?>>Laki-laki
                    </option>
                    <option value="P" <?= old('j_kelamin', $pasien['j_kelamin']) == 'P' ? 'selected' : '' ?>>Perempuan
                    </option>
                </select>
            </div>
        ...
        <div class="mt-5">
            ...
            <div class="relative mt-1">
                <input type="date" name="tgl_lahir" id="tgl_lahir"
                    value="<?= old('tgl_lahir') ? old('tgl_lahir') : date('Y-m-d', strtotime($pasien['tgl_lahir'])); ?>"
                    class="block w-full px-2 py-2 text-gray-500 bg-transparent border-b border-gray-400 focus:outline-none focus:border-black transition duration-200">
